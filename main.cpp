...
void printBoard(int uPos, int aiPos) {
  ...
  for (int i = 9; i >= 0; i--) {
    for (int j = 0; j < 10; j++) {
      ...
      // otherwise, if the cell is the computer's position, print the computer's initial 
      else if (cell == aiPos) {
        ...
        if (!isSnakeOrLadder) {
          for (int k = 0; k < ladderCount; k++) {
            if (cell == ladderPosUpper[k]) {
              cout << BRIGHT_MAGENTA << setw(4) << "L" + to_string(k + 1) << RESET;
              isSnakeOrLadder = true;
              break;
...
void checkSnakeLadderOverlap()
{
  for (int i = 0; i < snakeCount; i++)
  {
    for (int j = 0; j < ladderCount; j++)
    {
      ...
      if (snakePosUpper.at(i) == ladderPosUpper.at(j) || snakePosUpper.at(i) == ladderPosLower.at(j) || snakePosLower.at(i) == ladderPosUpper.at(j) || snakePosLower.at(i) == ladderPosLower.at(j))
      {
...
int checkSnakesLadders(int pos)
{
  for (int i = 0; i < snakeCount; i++)
  {
    if (pos == snakePosUpper.at(i))
    {
      ...
      if (usersTurn)
      {
        cout << name << " landed on a snake! " << name << " is now at position " << pos << endl;
      }
      else
      {
        cout << compName << " landed on a snake! " << compName << " is now at position " << pos << endl;
      }
      return pos;
    }
  }
  for (int i = 0; i < ladderCount; i++)
  {
    if (pos == ladderPosLower.at(i))
    {
      ...
      if (usersTurn)
      {
        cout << name << " landed on a ladder! " << name << " is now at position " << pos << endl;
      }
      else
      {
        cout << compName << " landed on a ladder! " << compName << " is now at position " << pos << endl;
      }
      return pos;
    }
  }
...
void turn()
{
  // clear screen using: https://stackoverflow.com/questions/17335816/clear-screen-using-c
